...
def _command_words(command: str) -> list[str]:
    """Return executable-position words from a shell command."""
    try:
...
def is_broad_discovery_command(command: str) -> bool:
    """Whether a shell command invokes a blocked broad-discovery executable."""
...
def _hook_classifier_script(tool_name: str, claude: bool) -> str:
    constants = (
        f"B={BROAD_DISCOVERY_COMMANDS!r};E={SOURCE_EXTENSIONS!r};I={IGNORED_SOURCE_PARTS!r};"
...
def _load_json_for_merge(path: Path) -> dict:
    ...
    try:
        ...
        warn(f"Backed up invalid JSON to {backup}")
        return {}
...
def configure_codex_project(project_dir: Path) -> None:
    ...
    if codex_dir.exists() and not codex_dir.is_dir():
        backup = _backup_path(codex_dir)
        warn(f"Backed up .codex file collision to {backup}")
