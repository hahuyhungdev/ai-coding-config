...
function getComponentStructureViolation(relativeFile) {
  ...
  if (parts[0] === "features" && parts[2] === "components" && parts.length >= 5) {
    if (fileName !== "index.tsx") {
      return "feature components must use components/Foo/index.tsx";
    }
  }

  if (!fs.existsSync(path.join(srcDir, "shared")) && parts[0] === "components") {
    if (parts.length === 2) {
      return "shared components must use components/Foo/index.tsx";
    }
    if (parts.length >= 3 && fileName !== "index.tsx") {
      return "shared components must use components/Foo/index.tsx";
    }
  }

  if (parts[0] === "shared" && parts[1] === "components" && parts.length >= 4) {
    if (fileName !== "index.tsx") {
      return "shared components must use shared/components/.../Foo/index.tsx";
    }
  }
