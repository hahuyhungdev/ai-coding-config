...
description: Enforce React feature-based architecture (shared -> features -> pages -> app) and naming conventions. Activate this skill whenever the user asks to create or refactor React components, hooks, api, or structures.
---

# React Feature-Based Architecture Skill

This skill enforces the architecture and design guidelines of a feature-based React codebase.
...
- **features/<feature-name>/**: Domain-specific files. CANNOT import from pages/app or other sibling features. Must only import from shared/ and feature-local files (using relative imports).
  - Internal structure:
    - `index.tsx` (or `index.ts` if not exporting React components): Public API (Compound Component export).
...
## 5. Helper Scaffolding Scripts

This skill provides automation scripts to generate folders and files conforming to these architectural patterns. Run them using python:

* **Generate a complete new Feature:**
  ```bash
  python3 <skill-path>/scripts/generate-feature.py <feature-name>
  ```
  *(Generates `src/features/<feature-name>` with private subfolders, custom hooks, API clients, and the public `index.ts` API).*

* **Generate a new Component inside a folder:**
  ```bash
  python3 <skill-path>/scripts/generate-component.py <ComponentName> <TargetDirectory> [--scss]
  ```
  *(Generates `<TargetDirectory>/<ComponentName>/<ComponentName>.tsx` with named/default exports, and optionally a `.module.scss` file).*

## 6. Templates

When creating new items, refer to the following boilerplate templates inside the skill directory:
- Component template: [ComponentTemplate.tsx](./templates/ComponentTemplate.tsx)
- Feature index template: [FeatureIndexTemplate.tsx](./templates/FeatureIndexTemplate.tsx)
- Hook template: [HookTemplate.ts](./templates/HookTemplate.ts)
- API template: [ApiTemplate.ts](./templates/ApiTemplate.ts)
