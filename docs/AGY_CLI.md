...
12. **Status Quota 100% Fallback (Signed Out / Workspace Trust)**: Running `agy status` returned `100%` fallback for all accounts because the background check runs in a sandbox directory. Without an `installation_id` file present, the `agy-bin` CLI treated the sandbox as a fresh install, showing the Terms of Service consent screen. In addition, running the status check from an untrusted working directory (like the user home folder `~`) triggered a "Do you trust the contents of this project?" prompt. Copying `installation_id` and implementing an auto-confirm logic for the trust prompt in `pty_client.py` resolved this, ensuring detailed quotas are successfully parsed under all conditions.
13. **Wrapper Router 'add' Command Exclusion**: Running `agy add` was incorrectly intercepted and routed as a typo because the subcommand `add` was missing from the recognized command list in `tools/agy/agy`. Added it to the list and added `test_add_commands` coverage to `test_agy_cli_e2e.py`.
14. **Rollover Context Injection Retried Session Bug**: The session rescue (quota rollover) mechanism successfully generated the conversation context (`.agy_progress.md`), but the wrapper's retry loop started the new session with the original command arguments instead of injecting the generated context history prompt. Added context prompt injection in the retry loop block.
15. **Windows Credential and PTY Compatibility**: Windows now uses a UTF-8 batch router, preserves the native binary as `agy-bin.exe`, clears the stale `gemini:antigravity` Credential Manager entry after account switches, and uses `pywinpty` for live quota checks. The active account token remains sourced from `~/.gemini/antigravity-cli/antigravity-oauth-token`.
16. **Windows Custom Commands Path Resolution**: Custom command files (such as `rotate.toml` and `rotate.md`) originally used hardcoded Unix paths (`~`) and python3 commands, which failed to execute on Windows. The installer (`install-agy.py`) now dynamically resolves the correct python executable and replaces placeholders with the absolute path using forward slashes, enabling `/rotate` to work flawlessly on Windows.
