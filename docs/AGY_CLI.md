...
12. **Status Quota 100% Fallback (Signed Out)**: Running `agy status` returned `100%` fallback for all accounts because the background check runs in a sandbox directory. Without an `installation_id` file present, the `agy-bin` CLI treated the sandbox as a fresh install, showing the Terms of Service consent screen and treating the user as signed out. Copying `installation_id` to the sandbox resolved this.
